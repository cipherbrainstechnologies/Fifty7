...
- 2025-11-20: Windows Streamlit sessions now reconfigure stdout/stderr to UTF-8 at startup so Unicode-heavy log statements (e.g., rupee symbols, status icons) no longer trigger `UnicodeEncodeError` in the console. `_ensure_utf8_console()` now falls back to wrapping the underlying buffer/detached stream so the fix also works when Streamlit/Colorama replace the default TextIO wrappers. `strategy_engine` also installs an ASCII-only logging filter on Windows so any remaining emoji are automatically stripped from log records whenever the console refuses UTF-8.
...
- 2025-11-20: Dashboard hero metrics now fetch the live option LTP for the most recent open trade and surface an `Active P&L` metric (points delta + rupee MTM). Users immediately see the order ID, current quote, and unrealized gain/loss even while Realized P&L remains zero.
- 2025-11-20: `LiveStrategyRunner` reconciles manual exits every cycle: it scans the broker’s trade book first, then falls back to the order book and net positions to detect when a user squares off from the broker app. Detected exits are logged with the observed fill (or latest quote), daily P&L is updated, monitors stop, and active trades disappear from the UI so stale entries never linger.
- 2025-11-20: Manual-exit reconciliation now treats “symbol missing from positions” as a zero balance, so trades squared off at the broker get mirrored locally (CSV + Postgres + dashboard) even when Angel immediately clears the position row. The runner uses the latest tick/REST quote as the settlement price and closes the trade on the very next cycle in both local and hosted deployments.
- 2025-11-21: `TradeLogger.update_trade_exit()` normalizes `order_id` to strings before matching, so SmartAPI order IDs that CSV stores as integers now reconcile correctly. This prevents open trades from rehydrating every cycle and keeps Active P&L + daily loss checks in sync after manual exits.
- 2025-11-20: Added a SmartAPI websocket tick streamer that subscribes to NIFTY and any open option tradingsymbols. Live ticks feed the dashboard’s Active P&L tile and keep the underlying LTP metric tick-by-tick without relying on REST polling.
- 2025-11-20: The tick streamer now injects the broker’s current JWT `auth_token` (along with the `feed_token`) when instantiating `SmartWebSocketV2`, and it aborts reconnect attempts if the session cannot be refreshed. This guards against SmartAPI’s November client change that required the auth token parameter and previously caused crash loops.
- 2025-11-20: Dashboard rehydration strips NaN/blank `tradingsymbol` entries from `logs/trades.csv`, and the tick streamer now ignores invalid identifiers entirely. No SmartAPI lookups fire for garbage symbols anymore, preventing `symboltoken` spam and rate-limit rejections during idle sessions.
- 2025-11-20: Manual-exit reconciliation now bootstraps from `logs/trades.csv`. On startup, `LiveStrategyRunner` seeds `_orders_to_signals` with any `status=open` trades and each cycle unions that list with the live tracker. Even after restarts, broker trade-book/order-book/position data gets replayed into the log so the dashboard can never drift from actual broker state.
- 2025-11-20: Trade logs now persist Angel One `tradingsymbol` values (and the live runner backfills missing ones from broker trade/order books). Manual exits, tick subscriptions, and Active P&L remain accurate even after backend restarts that previously reduced symbols to plain `NIFTY`.
- 2025-11-20: The dashboard’s Realized P&L metric now falls back to the CSV trade log whenever the Postgres snapshot is empty/unavailable, so hosted sessions continue to show realized gains/losses even if the database connection blips.
- 2025-11-20: The `💹 P&L Analysis` tab now mirrors that fallback—if database queries fail or return empty, it hydrates cumulative P&L and the daily trend chart from `logs/trades.csv`, so desktop/dev runs without Postgres still surface full analytics.
- 2025-11-20: Tradingsymbol canonicalization now enforces SmartAPI `{SYMBOL}{DDMMMYY}{STRIKE}{CE/PE}` formatting end-to-end. `TradeLogger`, `LiveStrategyRunner`, tick streaming, and manual-exit reconciliation normalize legacy entries (e.g., `NIFTY25NOV25C26100`) so broker trade-book matches always succeed and Active P&L tiles clear immediately after a manual square-off.
- 2025-11-20: `LiveStrategyRunner` now embeds an Active P&L tracker thread that samples open trades every ≤5 seconds, merges SmartAPI websocket ticks with REST fallbacks, exposes `get_active_pnl_snapshot()`, and feeds the dashboard’s hero metrics so hosted deployments always see up-to-date MTM + LTP without manual reruns. `PositionMonitor` consumes the same quote cache (via an injected LTP provider) before touching broker APIs, reducing rate-limit pressure.
