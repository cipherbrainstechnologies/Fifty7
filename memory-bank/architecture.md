...
- 2025-11-20: `LiveStrategyRunner` reconciles manual exits by polling the broker trade book each cycle. If a user squares off from the broker app, the runner now logs the exit, computes realized P&L, stops monitors, and clears the active signal so the UI never shows stale “open” trades.
